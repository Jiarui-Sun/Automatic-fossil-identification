...
def load_qiao(root, mode='train'):
    ...
    if mode == 'train':
        ...
        weight = class_weight.compute_class_weight(class_weight='balanced', classes=np.unique(labels), y=labels)
...
images, labels, lei, weight = load_qiao(r'.\database\re-shuangqiao-2', 'train')
db_train = tf.data.Dataset.from_tensor_slices((images, labels))
db_train = db_train.shuffle(1000).map(preprocess).batch(batchsz) 

images2, labels2, lei2, weight2 = load_qiao(r'.\database\re-shuangqiao-2', 'val')
db_val = tf.data.Dataset.from_tensor_slices((images2, labels2))
db_vall = db_val.map(preprocess)
db_val = db_val.map(preprocess).batch(batchsz)

images3, labels3, lei3, weight2 = load_qiao(r'.\database\re-shuangqiao-2', 'test')
...
model_checkpoint_callback = tf.keras.callbacks.ModelCheckpoint(
    filepath='VGG16/model/'+now+'/'+ str(trainable) + str(amount_min) + now + '.h5',
    monitor='val_accuracy',
    mode='max',
    save_best_only=True)

logdir = "VGG16/logs/" + now + "/train_data/"
file_writer = tf.summary.create_file_writer(logdir)

logdir = "VGG16/logs/" + now + "/images"
tensorboard_callback = keras.callbacks.TensorBoard(log_dir=logdir)
file_writer_cm = tf.summary.create_file_writer(logdir + '/cm')

log_dir = "VGG16/logs/" + now + "/fit"
tensorboard_callback = tf.keras.callbacks.TensorBoard(log_dir=log_dir, histogram_freq=1)

history1 = newnet.fit(db_train, validation_data=db_val, validation_freq=1, epochs=50,
                      callbacks=[early_stopping, reduce_lr], class_weight=weight)  #
...
plt.plot(np.arange(len(returns)), returns, label='training_accuracy')
plt.plot(np.arange(len(returns)), returns, 's')
